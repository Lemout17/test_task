...
class AppBrandingColors extends ThemeExtension<AppBrandingColors> {
  const AppBrandingColors({required this.color});

  final Color? color;

  @override
  AppBrandingColors copyWith({Color? color}) {
    return AppBrandingColors(color: color ?? this.color);
  }
  ...
  AppBrandingColors lerp(AppBrandingColors? other, double t) {
    ...
    return AppBrandingColors(color: Color.lerp(color, other.color, t));
  }
}
